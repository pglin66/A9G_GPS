...
#define CONFIG_FILE_NAME "/test.conf"

#define TF_CARD_TEST_STRING    "{'a':'1','b':'dddd'}"
...
bool ReadData(uint8_t *data,int length)
{
    ...
    ret = API_FS_Read(fd, data2, sizeof(data2)) ;

    Trace(1,"read ret:%d,sizeof(Data_t):%d",ret, length);
	Trace(2, "read ret data:%s", data2);
    API_FS_Close(fd);
    if(ret <= 0)
        return false;
	return true;
